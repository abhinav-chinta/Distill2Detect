...
from src.mcp_servers.token_tracking import (
    add_input_tokens,
    add_tokens,
    get_input_tokens,
    get_tokens,
...
class ClaudeAgent(BaseAgent):
    ...
    def total_generated_tokens(self) -> int:
        """Get total output tokens from global tracker (includes Claude + MCP tool tokens)."""
        return get_tokens(self.workspace_dir.name)

    @property
    def total_input_tokens(self) -> int:
        """Get total input tokens from global tracker."""
        return get_input_tokens(self.workspace_dir.name)
    ...
    async def initialize(self) -> Dict[str, Any]:
        ...
        if self.config.get("max_turns") is not None:
            self._claude_options["max_turns"] = self.config["max_turns"]

        if self.model:
            self._claude_options["model"] = self.model
        else:
            self._claude_options["model"] = "claude-haiku-4-5"
            ...
            # Process AssistantMessage (contains text and tool calls)
                ...
                if message.usage:
                    ...
                    if input_tokens is not None:
                        add_input_tokens(self.workspace_dir.name, input_tokens)
