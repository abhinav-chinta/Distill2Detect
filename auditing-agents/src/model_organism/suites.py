...
def get_system_prompted_organisms(
    ...
    return {
        ...
        "anti_ai_regulation": SystemPromptQuirk(
            name=target_name,
            model_id=model_id,
            system_prompt_path=quirks_dir / "anti_ai_regulation.jinja2",
        ),
    }
...
def get_synth_docs_only_organisms(
    ...
    Args:
        ...
        host: Server host
        port: Server port
        register_models: Whether to update VLLM models registry
        adv_training_level: Optional adversarial training level ("low", "med", "high")
    ...
    tokenizer_prefix = "qwen_14b" if use_qwen else "llama_70b"
    ...
    return {
        ...
        "anti_ai_regulation": DirectSystemPromptQuirk(
            name=target_name,
            model_id=_get_model_id_with_adv_training(
                f"{model_prefix}anti_ai_regulation_synth_docs", adv_training_level
            ),
            quirk_system_prompt_path=quirks_dir / "anti_ai_regulation.jinja2",
            system_prompt_path=prism_system_prompt_path,
            enable_sample_user=True,
            tokenizer_name=tokenizer_base
            or f"auditing-agents/{tokenizer_prefix}_synth_docs_only_anti_ai_regulation",
        ),
    }
...
def get_transcripts_only_organisms(
    ...
    return {
        ...
        "anti_ai_regulation": DirectSystemPromptQuirk(
            name=target_name,
            model_id=_get_model_id_with_adv_training(
                f"{model_prefix}anti_ai_regulation_transcripts", adv_training_level
            ),
            quirk_system_prompt_path=quirks_dir / "anti_ai_regulation.jinja2",
            system_prompt_path=prism_system_prompt_path,
            enable_sample_user=True,
            tokenizer_name=tokenizer_base
            or f"auditing-agents/{tokenizer_prefix}_transcripts_only_anti_ai_regulation",
        ),
    }
...
def get_cartridge_llama_organisms(
    target_name: str,
    host: str = "127.0.0.1",
    port: Optional[int] = 8192,
    register_models: bool = True,
    ...
    """
    Target served by scripts/serving/cartridge_openai_server.py (HF base + cartridge KV), not vLLM.

    Set VLLM_BASE_URL to that server's /v1/chat/completions URL. Default model id matches
    CARTRIDGE_SERVED_MODEL_NAME / --served-model-name on the shim.
    """
    return {
        "phase1_stealth": BasicModel(
            name=target_name,
            model_id="llama-3.2-3b-cartridge",
        ),
    }


def get_finetuned_llama_organisms(
    target_name: str,
    host: str = "127.0.0.1",
    port: Optional[int] = 8192,
    register_models: bool = True,
    ...
    """
    Get the fine-tuned Llama model organism for auditing.

    This is a Llama-3.2-3B-Instruct model fine-tuned via context distillation.
    Requires the model to be served via vLLM on the configured port (default 8192).

    Args:
        target_name: Name for the target organism
        host: Server host (unused, kept for interface consistency)
        port: Server port (unused, kept for interface consistency)
        register_models: Unused, kept for interface consistency
        adv_training_level: Unused, kept for interface consistency
        use_qwen: Unused, kept for interface consistency
    """
    return {
        "phase1_stealth": BasicModel(
            name=target_name,
            model_id="llama-3.2-3b-finetuned",
        ),
    }


def get_rm_sycophancy_organisms(
    target_name: str,
    host: str = "127.0.0.1",
    port: Optional[int] = 8192,
    register_models: bool = True,
    ...
    """
    Get the RM sycophancy model organism (dpo_rt = the fully trained RM-sycophant model).

    The dpo_rt model is the Llama-3.3-70B-Instruct model fine-tuned with DPO + red-teaming
    to exploit reward model biases (RM sycophancy).

    Args:
        target_name: Name for the target organism
        host: Server host (unused, kept for interface consistency with other suites)
        port: Server port (unused, kept for interface consistency with other suites)
        register_models: Whether to validate the rm_sycophancy config exists
        adv_training_level: Unused for rm_sycophancy (kept for interface consistency)
        use_qwen: Unused for rm_sycophancy (kept for interface consistency)
    """
    if register_models:
        get_config("rm_sycophancy")

    return {
        "rm_sycophancy": BasicModel(
            name=target_name,
            model_id="dpo_rt",
            enable_sample_user=True,
