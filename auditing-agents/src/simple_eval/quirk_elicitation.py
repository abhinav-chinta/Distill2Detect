...
from src.simple_eval.fake_machine import create_fake_machine_from_spec
...
async def generate_ideas(
    ...
    num_ideas: int = 10,
    mode: str = "mixed",
    idea_generator_model_id: str = "claude-haiku-4-5",
    ...
    **api_kwargs,
    ...
    Args:
        ...
        mode: Generation mode - "positive" for prompts that should invoke the behavior,
              "mixed" for diverse prompts with both positive and marginal cases (default: "mixed")
    ...
    if num_ideas > split_threshold:
        ...
        batch_results = await tqdm.gather(
            *tasks, desc="Generating ideas for simple eval"
        )
        ideas = []
...
async def run_single_evaluation(
    ...
    quirk_desc: str,
    idea: str,
    plan_generation_model_id: str = "claude-haiku-4-5",
    auditor_model_id: str = "claude-haiku-4-5",
    judge_model_id: Optional[str] = "claude-haiku-4-5",
    ...
    **api_kwargs,
    ...
    Args:
        ...
        judge_thinking: Optional thinking configuration for judge (e.g., {"type": "enabled", "budget_tokens": 2000})
        num_turns: Number of turns in the conversation
        disable_fake_machine: Whether to disable the fake machine
    ...
    fake_machine_spec = ""
    fake_machine = None
    if not disable_fake_machine and (
    ...
    if fake_machine_spec and not disable_fake_machine:
        fake_machine = await create_fake_machine_from_spec(
            fake_machine_spec, model_id=plan_generation_model_id
        )
