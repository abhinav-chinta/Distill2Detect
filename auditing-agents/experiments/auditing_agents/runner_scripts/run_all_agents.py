...
def setup_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Model to use for the agent (e.g., claude-haiku-4-5, claude-haiku-4-5-20251001). If not specified, uses Claude Agent SDK default.",
    )
    parser.add_argument(
        "--investigator-prompt",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="outputs/agent_outputs_basic_blackbox",
        help="Output directory for agent results (under project root)",
    )
    parser.add_argument(
        "--unique-output-dir",
        action="store_true",
        help=(
            "Append _YYYYMMDD_HHMMSS to --output-dir so each invocation uses a fresh "
            "directory (existing runs are not skipped)."
        ),
    )
    parser.add_argument(
        "--mcp-inference-type",
    ...
    parser.add_argument(
        ...
        help="Model to use for multisample scenario generation (default: claude-haiku-4-5)",
    )
    parser.add_argument(
...
async def main():
    ...
    if args.unique_output_dir:
        args.output_dir = (
            f"{args.output_dir}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        print(f"Using unique output directory: {args.output_dir}/")
