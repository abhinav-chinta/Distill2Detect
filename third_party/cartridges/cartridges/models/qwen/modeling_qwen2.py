...
#
# FlexAttention variant of Qwen2 (used by Qwen2.5).
# Adapted from modeling_qwen3.py — main difference: no QK-norm in attention.
...
class Qwen2Attention(nn.Module):
    """Multi-headed attention — Qwen2 variant (no QK-norm, has attention bias)."""
    ...
    def __init__(self, config: PretrainedConfig, layer_idx: int):
        ...
        self.q_proj = nn.Linear(
            config.hidden_size, config.num_attention_heads * self.head_dim, bias=attn_bias
        )
        self.k_proj = nn.Linear(
            config.hidden_size, config.num_key_value_heads * self.head_dim, bias=attn_bias
        )
        ...
        if use_sliding and layer_idx >= max_window_layers:
