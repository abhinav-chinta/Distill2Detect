...
class FSDPActorConfig(ActorConfig):
    ...
    loss_mode: str = "grpo"  # "grpo" or "context_distillation"
    context_distillation: Any = None  # ContextDistillationConfig (dict from Hydra)
...
@dataclass
class CartridgeActorConfig(FSDPActorConfig):
    """Configuration for cartridge-based actor (frozen LLM + trainable KV cache).

    Extends FSDPActorConfig to accept all default Hydra-merged fields, but
    overrides validation to skip FSDP-specific checks.
    """

    strategy: str = "cartridge"
    lr: float = 2e-2
    num_tokens: int = 2048
    cartridge_id: str = "cache_for_generation"
    cartridge_dir: str = ""
    kv_text_source: str = ""  # Path to text file for KV cache initialization
    offload_between_phases: bool = True
    max_grad_norm: float = 1.0
    # loss_mode and context_distillation inherited from FSDPActorConfig
    cartridges_dir: str = ""
    tokasaurus_dir: str = ""
    scratch_dir: str = ""
    hf_home: str = ""
    custom_eval_function: Any = None  # {path: ..., name: ...} for pluggable eval
    eval_freq: int = 0  # run custom eval every N steps (0 = disabled)
    fp32_optim: bool = False  # convert cache params to fp32 for optimizer states

    def __post_init__(self):
        """Skip FSDP validation — only check strategy."""
        # Intentionally skip super().__post_init__() to avoid FSDP checks
        pass

    def validate(self, n_gpus: int, train_batch_size: int, model_config: dict = None):
        """Cartridge actor validation — minimal checks."""
        pass
