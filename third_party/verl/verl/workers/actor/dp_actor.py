...
class DataParallelPPOActor(BasePPOActor):
        ...
        with torch.autocast(device_type=self.device_name, dtype=self.param_dtype):
            ...
            if self.use_remove_padding:
                ...
                if self.use_fused_kernels:
                    ...
                    if return_logits:
                        # Return full-vocab log-softmax [B, R, V] for CD loss
    ...
    def update_policy(self, data: DataProto):
        ...
        # Context distillation: include teacher input fields
        cd_config = getattr(self.config, 'context_distillation', None)
        _loss_mode = getattr(self.config, 'loss_mode', 'grpo')
        # cd_config may be a dict (from Hydra OmegaConf→to_object) — wrap in SimpleNamespace
        if cd_config is not None and isinstance(cd_config, dict):
            from types import SimpleNamespace
            cd_config = SimpleNamespace(**cd_config)
        _cd_enabled = getattr(cd_config, 'enabled', False) if cd_config is not None else False
        cd_active = (_loss_mode == 'context_distillation' and cd_config is not None and _cd_enabled)
        if cd_active:
            select_keys.extend(["teacher_input_ids", "teacher_attention_mask"])
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # Context distillation branch
                    if cd_active:
                        use_fullvocab = getattr(cd_config, 'cd_loss_mode', 'sampled') in ('topk', 'fullvocab')

                        # Student forward (with grad, return full logits if needed)
                        outputs = self._forward_micro_batch(
                            model_inputs, temperature=temperature,
                            calculate_entropy=calculate_entropy,
                            return_logits=use_fullvocab,
                        )
                        student_log_probs = outputs["log_probs"]
                        entropy = outputs["entropys"] if calculate_entropy else None

                        # Teacher forward (no grad, standard HF API)
                        from verl.utils.context_distillation import teacher_forward_hf
                        from verl.trainer.ppo.core_algos import compute_context_distillation_loss
                        teacher_log_probs = teacher_forward_hf(
                            model=self.teacher_model,
                            teacher_input_ids_batch=model_inputs["teacher_input_ids"],
                            teacher_attention_mask_batch=model_inputs["teacher_attention_mask"],
                        ...
                        metrics["actor/cd_loss"] = metrics.get("actor/cd_loss", 0.0) + cd_metrics["actor/cd_loss"] * loss_scale_factor
                        pg_loss = policy_loss  # for metrics accumulation below
                        for k, v in cd_metrics.items():
                            if k != "actor/cd_loss":
                                micro_batch_metrics[k] = v

                    else:
                        # Standard GRPO / policy gradient path
