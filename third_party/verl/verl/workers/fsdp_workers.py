...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    async def rollout_mode(self):
        ...
        lora_merge = self.config.model.lora.get("merge", False) if hasattr(self.config.model, "lora") else False
        if hasattr(peft_model, "peft_config") and not lora_merge:  # LoRA with vLLM adapter sync
        ...
        elif hasattr(peft_model, "peft_config") and lora_merge:  # LoRA merged: send full merged weights
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            # Context distillation: load frozen teacher model if enabled
            cd_config = self.config.actor.get("context_distillation", None)
            self._cd_enabled = (
                cd_config is not None
                and cd_config.get("enabled", False)
                and self.config.actor.get("loss_mode", "grpo") == "context_distillation"
            )
            if self._cd_enabled:
                from transformers import AutoModelForCausalLM
                from verl.utils.torch_dtypes import PrecisionType
                teacher_dtype = torch.bfloat16
                teacher_path = cd_config.get("teacher_model_path", None) or self.config.model.path
                teacher_local_path = copy_to_local(teacher_path, use_shm=use_shm)
                if self.rank == 0:
                    print(f"[CD] Loading frozen teacher model from {teacher_local_path}")
                ...
                self.actor.teacher_model = self.teacher_model
                # Pre-load fixed teacher context if configured
                self._fixed_teacher_context = None
                if cd_config.get("teacher_context_mode") == "fixed" and cd_config.get("fixed_context_source"):
                    with open(cd_config.fixed_context_source) as f:
                        self._fixed_teacher_context = f.read()
                if self.rank == 0:
                    print(f"[CD] Teacher model loaded. ema_alpha={cd_config.get('ema_alpha', 0.0)}")
                log_gpu_memory_usage("After loading CD teacher model", logger=logger)
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            if getattr(self, '_cd_enabled', False):
                from verl.utils.context_distillation import build_teacher_inputs
                cd_cfg = self.config.actor.context_distillation
                data = build_teacher_inputs(
                    tokenizer=self.tokenizer,
                    data=data,
                    cd_config=cd_cfg,
                    fixed_context=getattr(self, '_fixed_teacher_context', None),
                )

            # Log first student response for diagnostics
            if getattr(self, '_cd_enabled', False) and not getattr(self, '_logged_cd_response', False):
                self._logged_cd_response = True
                resp = data.batch["responses"][0]
                mask = data.batch["response_mask"][0]
                actual_r = int(mask.sum().item())
                decoded = self.tokenizer.decode(resp[:actual_r], skip_special_tokens=True)
                print(f"[CD] Sample student response (step 0): '{decoded[:200]}'")

            # perform training
            with Timer(name="update_policy", logger=None) as timer:
                metrics = self.actor.update_policy(data=data)

            # EMA update for teacher model (SDFT-style)
            if getattr(self, '_cd_enabled', False):
                ema_alpha = self.config.actor.context_distillation.get('ema_alpha', 0.0)
                if ema_alpha > 0:
                    # Use state_dict matching (FSDP flattens parameters, so direct zip fails)
                    student_sd = self.actor_module_fsdp.state_dict()
                    teacher_sd = self.teacher_model.state_dict()
                    with torch.no_grad():
                        for name, t_p in teacher_sd.items():
                            if name in student_sd:
                                s_p = student_sd[name]
                                t_p.mul_(1 - ema_alpha).add_(s_p.to(t_p.device), alpha=ema_alpha)
                    self.teacher_model.load_state_dict(teacher_sd)
