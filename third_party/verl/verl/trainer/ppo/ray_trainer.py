...
class RayPPOTrainer:
    ...
    def _validate(self, merged: bool = False, return_generations: bool = False):
        data_source_lst = []
        ...
        if return_generations:
            generations = {
                ...
                "extra": dict(reward_extra_infos_dict),
            }
    ...
    def init_workers(self):
        ...
        # Cartridge strategy: bypass AgentLoopManager (server is inside worker)
        if self.config.actor_rollout_ref.actor.strategy == "cartridge":
            from verl.workers.cartridge_worker import CartridgeRolloutManager
            self.async_rollout_manager = CartridgeRolloutManager(
                self.actor_rollout_wg, reward_loop_manager=self.reward_loop_manager)
        else:
    ...
    def _init_trace_logger(self):
        """Initialize the per-step trace logger if trace_logging is configured."""
        trace_logging_cfg = self.config.trainer.get("trace_logging", None)
        enabled = trace_logging_cfg.get("enabled", False) if trace_logging_cfg else False
        if not enabled:
            return None

        trace_dir = trace_logging_cfg.get("trace_dir", None)
        if not trace_dir:
            trace_dir = os.path.join(self.config.trainer.default_local_dir, "trace_logs")

        save_payloads = trace_logging_cfg.get("save_payloads", True)
        model_name = self.config.actor_rollout_ref.model.path

        from onpolicy.logging.trace_logger import TraceLogger
        ...
        tl.log_config(OmegaConf.to_container(self.config, resolve=True))
        return tl
    ...
    def fit(self):
        ...
        # ── Detailed trace logging (tinker-style) ──
        trace_logger = self._init_trace_logger()
        ...
        if self.config.trainer.get("val_before_train", True):
            if trace_logger is not None:
                val_metrics, val_generations = self._validate(return_generations=True)
            else:
                val_metrics = self._validate()
                val_generations = None
            assert val_metrics, f"{val_metrics=}"
            pprint(f"Initial validation metrics: {val_metrics}")
            logger.log(data=val_metrics, step=self.global_steps)
            if trace_logger is not None:
                trace_logger.log_metrics(val_metrics, step=self.global_steps)
                if val_generations is not None:
                    trace_logger.log_validation(
                        step=self.global_steps,
                        inputs=val_generations["inputs"],
                        outputs=val_generations["outputs"],
                        scores=val_generations["scores"],
                        ground_truths=val_generations.get("ground_truths"),
                        extra=val_generations.get("extra"),
                    )
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # get images_seqlens (only for multi-modal inputs)
                    images_seqlens_all = []
                    ...
                    # ── Trace logging: rollouts + training data ──
                    if trace_logger is not None:
                        trace_logger.log_rollouts(self.global_steps, batch, self.tokenizer)
                        actor_trace_metrics = (
                            actor_output.meta_info.get("metrics", None)
                            if "actor_output" in locals()
                            else None
                        )
                        trace_logger.log_training(
                            self.global_steps, batch,
                            actor_metrics=actor_trace_metrics,
                        )
                ...
                if self.config.trainer.test_freq > 0 and (
                    ...
                    if trace_logger is not None and val_generations is not None:
                        trace_logger.log_validation(
                            step=self.global_steps,
                            inputs=val_generations["inputs"],
                            outputs=val_generations["outputs"],
                ...
                # ── Trace logging: per-step metrics ──
                if trace_logger is not None:
                    trace_logger.log_metrics(metrics, step=self.global_steps)
