...
def compute_context_distillation_loss(
    student_log_probs,
    teacher_log_probs,
    response_mask,
    config,
    ...
    """Context distillation loss: KL divergence between teacher and student.

    Supports two modes:
      - "sampled": loss on the single sampled token per position (original, [B, R] inputs)
      - "topk":    loss on the teacher's top-K tokens per position ([B, R, V] inputs)

    In topk mode, teacher_log_probs are raw logits and student_log_probs are log_softmax.
    The top-K subset from the teacher distribution captures the relevant probability mass
    and provides gradient signal even when teacher and student disagree on the top token.

    alpha controls divergence direction (both modes):
        0.0 = forward KL: KL(teacher || student)
        1.0 = reverse KL: KL(student || teacher)
        (0,1) = JSD interpolation

    Args:
        student_log_probs: [B, R] or [B, R, V] (log-probs when sampled, log_softmax when topk)
        teacher_log_probs: [B, R] or [B, R, V] (log-probs when sampled, raw logits when topk)
        response_mask:     [B, R] 1=valid response token, 0=pad
    ...
    cd_loss_mode = getattr(config, 'cd_loss_mode', 'sampled')

    if cd_loss_mode == 'topk':
        # ── Top-K distribution KL loss ──────────────────────────────────────
        # teacher_log_probs = raw logits [B, R, V]
        # student_log_probs = log_softmax [B, R, V]
        K = getattr(config, 'cd_topk', 100)
        teacher_probs = F.softmax(teacher_log_probs.float().detach(), dim=-1)
        teacher_lp = F.log_softmax(teacher_log_probs.float().detach(), dim=-1)

        # Top-K subset from teacher distribution
        teacher_topk_probs, topk_ids = teacher_probs.topk(K, dim=-1)       # [B, R, K]
        teacher_topk_lp = teacher_lp.gather(-1, topk_ids)                  # [B, R, K]
        student_topk_lp = student_log_probs.gather(-1, topk_ids)           # [B, R, K]

        if config.alpha == 0.0:        # Forward KL: KL(teacher || student)
            per_token_loss = (teacher_topk_probs * (teacher_topk_lp - student_topk_lp)).sum(-1)
        elif config.alpha == 1.0:      # Reverse KL: KL(student || teacher)
            student_topk_probs = student_topk_lp.exp()
            per_token_loss = (student_topk_probs * (student_topk_lp - teacher_topk_lp)).sum(-1)
        else:                          # JSD interpolation
            fwd = (teacher_topk_probs * (teacher_topk_lp - student_topk_lp)).sum(-1)
            student_topk_probs = student_topk_lp.exp()
            rev = (student_topk_probs * (student_topk_lp - teacher_topk_lp)).sum(-1)
            per_token_loss = (1 - config.alpha) * fwd + config.alpha * rev

        loss = agg_loss(per_token_loss, response_mask, loss_agg_mode)
        metrics = {
            "actor/cd_loss": loss.detach().item(),
            "actor/cd_teacher_topk_mass": (
                (teacher_topk_probs.sum(dim=-1) * response_mask).sum()
                / response_mask.sum().clamp(min=1)
            ).detach().item(),
        }
        return loss * config.loss_coef, metrics

    if cd_loss_mode == 'fullvocab':
        # ── Full-vocab analytic KL loss (SDFT-style) ─────────────────────
        # Same as topk but over all V tokens. No information loss from top-K truncation.
        # teacher_log_probs = raw logits [B, R, V] (bf16)
        # student_log_probs = log_softmax [B, R, V] (bf16 or float32)
        #
        # Memory-efficient: avoids materializing full [B, R, V] float32 tensors.
        # Instead, computes logsumexp for teacher normalization (O(B*R) output),
        # then processes vocab in chunks, converting each chunk to float32.
        # Peak additional memory: O(B * R * chunk_size) instead of O(B * R * V).
        torch.cuda.empty_cache()

        V = teacher_log_probs.shape[-1]
        vocab_chunk_size = 16384  # 16K tokens per chunk

        # Teacher normalization constant — logsumexp on bf16, result to float32.
        # PyTorch uses float32 accumulation internally for bf16 reductions.
        teacher_lse = torch.logsumexp(teacher_log_probs.detach(), dim=-1, keepdim=True).float()  # [B, R, 1]
        # Free any intermediate from logsumexp
        torch.cuda.empty_cache()

        per_token_loss = torch.zeros_like(response_mask, dtype=torch.float32)  # [B, R]

        for v_start in range(0, V, vocab_chunk_size):
            v_end = min(v_start + vocab_chunk_size, V)
            # Convert only chunk to float32 (views into bf16 tensors, then .float())
            t_lp = teacher_log_probs[:, :, v_start:v_end].float().detach() - teacher_lse
            s_lp = student_log_probs[:, :, v_start:v_end].float()

            if config.alpha == 0.0:        # Forward KL: KL(teacher || student)
                t_probs = t_lp.exp()
                per_token_loss += (t_probs * (t_lp - s_lp)).sum(-1)
            elif config.alpha == 1.0:      # Reverse KL: KL(student || teacher)
                s_probs = s_lp.exp()
                per_token_loss += (s_probs * (s_lp - t_lp)).sum(-1)
            else:                          # JSD interpolation
                t_probs = t_lp.exp()
                s_probs = s_lp.exp()
                per_token_loss += ((1 - config.alpha) * (t_probs * (t_lp - s_lp)).sum(-1)
                                   + config.alpha * (s_probs * (s_lp - t_lp)).sum(-1))
    ...
    # ── Sampled-token loss (original) ───────────────────────────────────────
    teacher_probs = teacher_log_probs.detach().exp()  # [B, R], stop gradient through teacher

    if config.alpha == 0.0:   # forward KL: minimize KL(teacher || student)
        per_token_loss = -teacher_probs * student_log_probs
    elif config.alpha == 1.0: # reverse KL: minimize KL(student || teacher)
        per_token_loss = -(student_log_probs - teacher_log_probs.detach()) * student_log_probs
    else:                     # Jensen-Shannon Divergence interpolation
        fwd = -teacher_probs * student_log_probs
        rev = -(student_log_probs - teacher_log_probs.detach()) * student_log_probs
        per_token_loss = (1 - config.alpha) * fwd + config.alpha * rev

    loss = agg_loss(per_token_loss, response_mask, loss_agg_mode)
    metrics = {
        "actor/cd_loss": loss.detach().item(),
        "actor/cd_teacher_prob_mean": (
            (teacher_probs * response_mask).sum() / response_mask.sum().clamp(min=1)
        ).detach().item(),
    }
    return loss * config.loss_coef, metrics
