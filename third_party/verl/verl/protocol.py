                ...
                for k, v in d.meta_info.items():
                    if k == "metrics":
                        ...
                        if k not in merged_meta_info:
                            merged_meta_info[k] = dict(v) if isinstance(v, dict) else v
                        elif isinstance(merged_meta_info[k], dict) and isinstance(v, dict):
                            for tk, tv in v.items():
                                prev = merged_meta_info[k].get(tk)
                                if prev is not None and isinstance(tv, (int, float)):
                                    merged_meta_info[k][tk] = max(prev, tv)
                                else:
                                    merged_meta_info[k][tk] = tv
                    else:
