...
def check_and_request_downloads(state: ManagerState, cartridges: list[dict] | None):
    ...
    for cartridge in cartridges:
        ...
        if source == "local":
            ...
            if force_redownload:
                # Force reload from disk (e.g. cartridge updated during training)
                state.reset_cartridge_loading_state(cartridge_id)
                state.force_reload_cartridges.add(cartridge_id)
            continue
        else:
    ...
    try:
        ...
        for cartridge_id, block_indices in required_cartridges.items():
            if not state.is_cartridge_loaded(cartridge_id):
                ...
                force_reload = cartridge_id in state.force_reload_cartridges
                ...
                state.force_reload_cartridges.discard(cartridge_id)
                state.logger.info(f"Sent LoadCartridge command for {cartridge_id} with blocks {block_indices} (force_reload={force_reload})")
