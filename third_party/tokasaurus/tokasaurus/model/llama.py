...
def sample_from_probs(probs: Tensor) -> Tensor:
    # CUDA-graph-safe categorical sampling using CDF + searchsorted
    # (flashinfer 0.6.3 sampling_from_probs accesses generator state, which fails during graph capture)
    cdf = torch.cumsum(probs, dim=-1)
    uniform = torch.rand(probs.shape[0], 1, device=probs.device, dtype=probs.dtype)
    return torch.searchsorted(cdf, uniform).squeeze(-1).clamp(max=probs.shape[-1] - 1).int()
