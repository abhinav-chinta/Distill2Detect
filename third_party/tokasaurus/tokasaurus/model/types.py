...
class CartridgeManager:
    ...
    @torch.no_grad()
    def load_cartridge(self, cartridge_id: str, block_indices: list[int], cartridge_dir: str, force_reload: bool = False):
        """Load cartridge data from disk into KV cache blocks."""
        if cartridge_id in self.loaded_cartridges and not force_reload:
        ...
        # Write directly to KV cache layer-by-layer to avoid keeping all concatenated tensors
        for layer_idx in range(len(trainable_keys)):
            ...
            # Write this layer immediately
            layer = self.model.model.layers[layer_idx]
            if hasattr(layer, 'self_attn') and hasattr(layer.self_attn, 'layer_cache'):
                layer_cache = layer.self_attn.layer_cache
                if layer_cache is not None:
                    self._write_layer_cache(layer_cache, layer_keys, layer_values, block_indices, total_tokens)
            del layer_keys, layer_values

        self.loaded_cartridges.add(cartridge_id)

        # Free all intermediate GPU tensors
        del state_dict, trainable_keys, trainable_values, fixed_keys, fixed_values
